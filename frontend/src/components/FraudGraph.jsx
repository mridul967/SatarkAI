...
export default function FraudGraph({ userId, API_URL }) {
  ...
  const renderGraph = (data) => {
    ...
    const link = svg.append('g').selectAll('line').data(data.links).enter().append('line')
      .attr('stroke', 'url(#link-grad)').attr('stroke-width', 1.5).attr('stroke-opacity', 0.4);

    const nodeGroup = svg.append('g').selectAll('g').data(data.nodes).enter().append('g').attr('class', 'cursor-pointer group')
      .call(d3.drag().on('start', dragstarted).on('drag', dragged).on('end', dragended));

    nodeGroup.append('circle').attr('r', d => d.group === 1 ? 22 : 0).attr('fill', 'var(--primary-glow)').attr('filter', 'blur(10px)').attr('opacity', 0.5);

    nodeGroup.append('circle').attr('r', d => d.group === 1 ? 16 : d.group === 4 ? 7 : 11)
      .attr('fill', d => d.group === 1 ? '#10b981' : d.group === 2 ? '#3b82f6' : d.group === 3 ? '#8b5cf6' : d.risk > 0.5 ? '#ff4d4d' : '#334155')
      .attr('stroke', 'rgba(255,255,255,0.1)').attr('stroke-width', 2)
      .style('filter', d => (d.group === 1 || d.risk > 0.5) ? 'url(#node-glow)' : 'none');

    nodeGroup.append('text').text(d => d.label).attr('dy', d => d.group === 1 ? 30 : 25).attr('text-anchor', 'middle')
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <div className="absolute top-4 right-6 flex items-center space-x-4 z-10 text-[9px] font-bold uppercase text-slate-500 tracking-widest">
        {['User', 'Entity', 'Risk'].map((l, i) => (
          <span key={l} className="flex items-center gap-1.5"><span className={`w-2 h-2 rounded-full ${['bg-emerald-500 glow-emerald', 'bg-blue-500', 'bg-red-500 glow-red'][i]}`}></span>{l}</span>
        ))}
      </div>
      {error && <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-red-500 text-xs font-mono">{error}</div>}
      {nodeCount === 0 && !error && (
        <div className="absolute inset-0 flex items-center justify-center text-slate-600 text-[10px] uppercase font-bold tracking-widest">Synchronizing Neural Fabric...</div>
      )}
